...
slack = Slacker('xoxb-1708188576007-1708189939271-jC4oeYG8l0YRNLsKqETERJLr')
def dbgout(message):
...
def get_target_price(code):  # 목표값 + 딥러닝
    """매수 목표가를 반환한다."""
    try:
...
def buy_etf(code): #산 종목
    ...
    try:
        ...
        target_price = get_target_price(code)    # 매수 목표가   변동성 전략
...
if __name__ == '__main__': 
    try:  #symbol_list : 종목코드
        ...
        while True:
            ...
            t_start = t_now.replace(hour=9, minute=5, second=0, microsecond=0) #시작시간
            t_sell = t_now.replace(hour=15, minute=15, second=0, microsecond=0) #파는시간
            t_exit = t_now.replace(hour=15, minute=20, second=0,microsecond=0) # 끝나는시간
            today = datetime.today().weekday()
            if today == 5 or today == 6:  # 토요일이나 일요일이면 자동 종료
                printlog('Today is', 'Saturday.' if today == 5 else 'Sunday.')
                sys.exit(0)
            if t_9 < t_now < t_start and soldout == False:  #9시와 9시 05분 사이에 남은 주식이 있으면 모두 팔라
                soldout = True
                sell_all()
